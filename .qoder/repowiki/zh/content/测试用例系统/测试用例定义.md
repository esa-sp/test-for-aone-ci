...
## 简介
本文件面向“测试用例定义”场景，系统性阐述 case.json 的数据模型与使用规范，涵盖字段含义、数据类型、取值范围、默认值与约束条件，并给出 JSON Schema 定义、字段验证规则、典型配置示例、执行顺序与依赖关系说明，以及最佳实践与常见错误模式。case.json 是 TestStep CI 测试编排的“用例总表”，每条用例在 CI 中逐条执行，依据声明的“期望日志片段”和“最终状态”判断通过或失败。

## 项目结构
- 顶层 case.json：测试用例数组，每条用例描述一次具体的测试场景。
...
```mermaid
graph TB
A["顶层仓库"] --> B["case.json<br/>测试用例数组"]
A --> C["README.md<br/>使用说明"]
A --> D[".claude/skills/add-test-case/SKILL.md<br/>技能文档"]
A --> E["backend-tests/README.md<br/>后端生成物验证说明"]
B --> F["每条用例对象<br/>name/envs/repoName/requireStatus/requireLogTextList/notRequireLogTextList"]
```
...
- 数据结构
  - case.json 是一个 JSON 数组，数组元素为“测试用例对象”。每个用例对象包含以下字段：
...
```mermaid
flowchart TD
Start(["开始"]) --> Parse["解析 case.json<br/>逐条读取用例"]
...
- 常见问题与解决思路
  ...
  - 用例顺序不当：用例间无强制依赖，但建议按“happy path → 边界条件 → 失败场景”的顺序组织，便于定位问题。
- 诊断步骤
...
## 结论
case.json 通过明确的字段定义与严格的断言规则，为 TestStep 的端到端测试提供了稳定可靠的编排基础。遵循“最小差异”原则创建 fixture、使用 <<LOG>>step|...<</LOG>> 标记作为断言锚点、合理组合 requireStatus 与日志断言，是保证用例质量与可维护性的关键。建议在新增用例前先查阅技能文档与现有用例，避免重复造轮子并降低维护成本。
