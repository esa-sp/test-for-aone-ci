...
```mermaid
graph TB
Root["backend-tests 根目录"] --> Egg["egg/"]
Root --> Elysia["elysia/"]
...
Midway --> MWMeta["meta.json"]
Midway --> MWBootstrap["bootstrap.js"]
Midway --> MWDist["dist/"]
Midway --> MWSrc["src/"]
Midway --> MWPkg["package.json"]
NestJS --> NSMain["src/main.ts"]
NestJS --> NSDist["dist/"]
NestJS --> NSMeta["meta.json"]
NestJS --> NSPkg["package.json"]
Elysia --> ElysiaApp["app.js"]
Elysia --> ElysiaMeta["meta.json"]
Elysia --> ElysiaPkg["package.json"]
```
...
WaitReady --> Assert["执行 assertions"]
...
ExportFetch --> Assert["执行 assertions"]
Assert --> Health["/api/health -> 200 + JSON 子集校验"]
Assert --> Users["/api/users/:id -> 200 + JSON 子集校验"]
Assert --> Echo["/api/echo POST -> 200 + JSON 子集校验"]
Health --> End(["结束"])
...
## 结论
backend-tests 提供了针对 framework-checker 生成物的“真跑”验证能力，通过 direct 与 spawn 两种模式覆盖主流后端框架，结合 meta.json 的断言定义与文件包含策略，确保构建产物在本机上的正确性、HTTP 响应的准确性以及运行时行为的稳定性。其设计简洁、执行高效，适合在 CI 中快速反馈问题。
