...
## 更新摘要
**所做更改**
- **重大变更**：移除了NestJS框架的Function Compute部署支持，删除了fc/目录及其所有配置文件
- **架构简化**：NestJS现在仅支持传统服务器部署模式，专注于企业级应用的标准部署方式
- **文档清理**：移除了所有与Function Compute相关的章节和配置说明
- **焦点调整**：文档现在更专注于三种框架的传统服务器部署模式和核心特性对比
...
- 构建流程与传统服务器部署策略（基于backend-tests目录中的测试夹具与断言）
- 框架检测机制的实现细节（通过代码特征识别框架类型）
- 各框架优缺点、适用场景与迁移建议
- 演示页面采用静态HTML结构，直接展示各框架的核心特性和编码模式
- 每个框架展示其特定的品牌化设计和界面风格

## 项目结构
本仓库包含三类测试夹具与配套断言，采用统一的静态演示页面架构：
- Koa应用夹具：位于backend-tests/koa目录，包含最小可运行示例、断言元数据和演示页面，展示Koa中间件模式
- Hono应用夹具：位于backend-tests/hono目录，包含最小可运行示例、断言元数据和演示页面，突出Hono链式API特性
- NestJS应用夹具：位于backend-tests/nestjs目录，包含TypeScript源码与编译产物，断言元数据指向dist产物，体现NestJS装饰器风格
...
- [backend-tests/koa/public/index.html:1-69](file://backend-tests/koa/public/index.html#L1-L69)
...
- [backend-tests/hono/public/index.html:1-69](file://backend-tests/hono/public/index.html#L1-L69)
...
- [backend-tests/nestjs/src/app.module.ts:1-18](file://backend-tests/nestjs/src/app.module.ts#L1-L18)
- [backend-tests/nestjs/src/app.controller.ts:1-40](file://backend-tests/nestjs/src/app.controller.ts#L1-L40)
...
## 架构总览
下图展示了三种夹具从"入口文件"到"HTTP响应"的关键流程，以及统一的静态演示页面架构。新的架构消除了复杂的动态渲染逻辑，采用更直接的静态HTML结构。
...
- 项目结构与入口
  - 使用Koa实例并通过监听端口启动
  - 集成路由与请求体解析中间件
  - 演示页面采用静态HTML结构，直接展示Koa中间件模式的实际代码示例
  - 品牌化设计：采用Koa标志性的绿色主题配色方案
- 关键实现要点
  - 中间件顺序：请求体解析需在路由之前
  - 路由设计：统一在根路径下提供健康检查、参数路由与POST回显
  - 界面亮点：静态HTML中嵌入实际的Koa代码示例，展示ctx.body设置响应体的模式
...
- 项目结构与入口
  ...
  - 路由以上下文形式返回JSON响应
  - 演示页面展示Hono链式API的实际编码模式
  - 品牌化设计：采用Hono特色的橙色主题配色方案
  - 自定义静态文件服务：采用fs.readFileSync实现替代@hono/node-server/serve-static中间件
- 关键实现要点
  - 界面亮点：静态HTML中嵌入实际的Hono代码示例，展示c.json()链式API模式
...
- 项目结构与入口
  ...
  - 演示页面突出NestJS装饰器驱动的开发模式
  - 品牌化设计：采用NestJS标志性的红色主题配色方案
- 关键实现要点
  - 模块化：通过装饰器声明模块与控制器
  - 控制器：统一在/api路径下提供健康检查、参数路由与POST回显
  - 界面亮点：静态HTML中嵌入实际的NestJS装饰器代码示例，展示@Controller、@Get、@Param等企业级特性
...
+echo(body : any) any
+listItems() any
+createItem(dto : CreateItemDto) any
}
class NestFactory {
...
### 演示页面架构
三个框架均采用统一的静态演示页面架构，通过各自的服务端配置提供静态文件服务：

- **静态HTML结构**：每个框架的演示页面都是独立的静态HTML文件，包含完整的样式和交互逻辑
- **框架特色展示**：直接在HTML中嵌入实际的框架代码示例，直观展示各框架的核心特性
- **品牌化设计**：每个框架都有独特的配色方案和视觉风格
- **简化交互逻辑**：保留API测试功能，采用简洁的JavaScript实现
- **模板系统参考**：_shared目录保留原始模板文件作为参考
...
- **服务配置**：
  - Koa：通过koa-static中间件提供静态文件服务
  - Hono：采用自定义fs.readFileSync实现，提供更好的安全性和可控性
  - NestJS：通过@nestjs/serve-static模块提供服务
- **资源优化**：支持缓存策略和压缩优化

```mermaid
graph TB
SharedTemplate["_shared/demo-page.template.html<br/>原始PAGE_CONFIG模板"] --> KoaStatic["Koa静态HTML<br/>绿色主题 + 中间件示例"]
SharedTemplate --> HonoStatic["Hono静态HTML<br/>橙色主题 + 链式API示例"]
SharedTemplate --> NestStatic["NestJS静态HTML<br/>红色主题 + 装饰器示例"]
KoaStatic --> KoaService["Koa静态服务<br/>koa-static中间件"]
HonoStatic --> HonoService["Hono静态服务<br/>自定义fs实现"]
NestStatic --> NestService["NestJS静态服务<br">@nestjs/serve-static"]
```
...
- [backend-tests/hono/public/index.html:1-69](file://backend-tests/hono/public/index.html#L1-L69)
- [backend-tests/nestjs/public/index.html:1-73](file://backend-tests/nestjs/public/index.html#L1-L73)
- [backend-tests/koa/server.js:19-20](file://backend-tests/koa/server.js#L19-L20)
- [backend-tests/hono/app.js:32-50](file://backend-tests/hono/app.js#L32-L50)
- [backend-tests/nestjs/src/app.module.ts:8-13](file://backend-tests/nestjs/src/app.module.ts#L8-L13)

**章节来源**
- [backend-tests/_shared/demo-page.template.html:1-227](file://backend-tests/_shared/demo-page.template.html#L1-L227)
- [backend-tests/_shared/demo-page.css:1-50](file://backend-tests/_shared/demo-page.css#L1-50)
- [backend-tests/koa/public/index.html:1-69](file://backend-tests/koa/public/index.html#L1-L69)
- [backend-tests/hono/public/index.html:1-69](file://backend-tests/hono/public/index.html#L1-L69)
- [backend-tests/nestjs/public/index.html:1-73](file://backend-tests/nestjs/public/index.html#L1-L73)
...
### 框架特定测试端点
- **Koa**：基础REST API测试，包括健康检查、用户查询、请求回显
- **Hono**：轻量级API测试，突出fetch风格和链式API
- **NestJS**：企业级API测试，包含资源管理、DTO验证、装饰器特性
...
- 依赖管理
  ...
  - Hono夹具：依赖hono、@hono/node-server（仅用于函数计算适配器），采用自定义静态文件服务实现
...
NestServeStatic["@nestjs/serve-static"] --> NestApp
StaticFiles["静态文件服务"] --> KoaApp
StaticFiles --> HonoApp
StaticFiles --> NestApp
DemoPages["静态演示页面"] --> KoaApp
DemoPages --> HonoApp
DemoPages --> NestApp
...
- 启动时间与预热
  ...
  - 静态HTML结构减少了前端渲染开销，提升页面加载速度
- 路由与中间件
  - Koa通过中间件链路处理请求，合理安排中间件顺序可减少不必要的解析开销
  - Hono以fetch风格直接返回响应，适合边缘计算与Web适配器场景，自定义静态文件服务避免了第三方中间件的额外开销
  - 静态文件优化：重构后的静态HTML文件更小，传输效率更高
- 构建与打包
  - NestJS需要编译TS到JS，建议在CI中缓存依赖与编译产物以提升速度
  - 消除了复杂的前端渲染逻辑，减少打包体积和运行时开销
...
## 结论
- Koa：轻量、灵活，适合快速搭建REST API与中间件生态丰富场景，界面重构后更好地展示了中间件模式的核心特性
- Hono：极简、原生fetch风格，适合边缘与云函数环境，界面重构突出了链式API的优雅设计，自定义静态文件服务提供了更好的安全性和性能
- NestJS：企业级架构，模块化与装饰器带来强约束与高可维护性，但需要编译与初始化成本，界面重构完美体现了装饰器驱动的开发模式，专注于传统服务器部署模式
- 架构演进：三者均已在本仓库夹具中通过断言验证，演示页面的统一架构代表了从复杂动态渲染到简洁静态结构的演进，提升了现代Web应用的交互体验

## 附录
- 框架检测机制
  - 顶层case.json展示了对多种后端框架的检测与打包行为，包括Express、Hono、Koa、NestJS等
  - 检测关注点：入口文件选择、是否包含框架依赖、是否包含特定目录结构（如/api）
  - 静态文件服务：各框架的静态文件服务实现差异
- 运行与接入
  - 通过blackBox入口脚本运行夹具测试，支持单夹具运行与批量运行
  - 退出码约定：0表示全部断言通过，1表示至少一个断言失败或启动失败
  - 界面访问：重构后的静态演示页面可通过浏览器直接访问，提供更直观的测试体验
  - 部署模式：所有框架均支持传统的服务器部署模式
